...
// kiểm tra trình duyêtj có hỗ trợ full màn hình không
function fullscreenEnabled() {
	return fscreen.fullscreenEnabled;
}

// kiểm tra có đang ở chế độ full màn hình không
function isFullscreen() {
	return !!fscreen.fullscreenElement;
}

// bật hoặc tắt chế độ full màn hình
...
function init() {
	...
	// shell type
	let options = '';
	options += '<option value="Random" selected>Random</option>';
	...
	// const defaultIndex = ['3"', '4"', '6"', '8"', '12"', '16"'].length - 1;
	// options += `<option value="${defaultIndex}" selected>${['3"', '4"', '6"', '8"', '12"', '16"'][defaultIndex]}</option>`;
