...
document.querySelector(".buttonLove").addEventListener("click", function(){
    let timeout = setInterval(()=>{
        ...
        letters.addEventListener("click", function () {
            ...
            //console.log(srcPath);
            const apiUrl = "https://erp.vnconference.com/api/method/api_lt.api.post.countdown2025.get_random_row";
            fetch(apiUrl)
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        ...
                        text_lixi = "Lì xì số: "+name;

                    } else if (data.status === 'no record') {
                        text_lixi = "Chúc mừng năm mới";
                        content = "Năm mới an khang thịnh vượng, sức khỏe dồi dào, gia đình hạnh phúc.";
                    } 
                    else {
                        text_lixi = "Chúc mừng năm mới";
                    }
                    textLetterH2 = "Bức Thư Số: "+name;
                    textLetterP = content;
                    return order;
                })
                .catch(error => {
